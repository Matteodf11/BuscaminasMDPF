...
public class AñadirController   {
    ...
    private void salvarAction(ActionEvent event) {
        Button clickedButton = (Button) event.getSource();
         ...
         if (clickedButton.getId().equals("salvar")) {
            if (!nombrepersona.getText().isEmpty() && !imagenpersona.getText().isEmpty()) {
                ...
                for (Persona autor1 : datosObservableList) {
                    if (autor1.getNombre().equals(nombrepersona.getText())) {
                        Alert alert = new Alert(Alert.AlertType.ERROR);
                        alert.setTitle("Jugador Duplicado");
                        alert.setHeaderText(null);
                        alert.setContentText("Ese jugador ya existe");
        ...
        datosObservableList.add(getPersona());
